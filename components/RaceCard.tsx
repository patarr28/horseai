...
export default function RaceCard({ race }: RaceCardProps) {
    ...
    return (
        <Link href={`/races/${race.id}`} className="group block">
            <div
                className={`relative overflow-hidden rounded-2xl glass-panel liquid-sheen caustic-spot border-l-4 ${borderColor} shadow-xl transition-all duration-300 ${glowClass} hover:border-surface-border-bright active:scale-[0.985] active:brightness-95 hover:shadow-2xl`}
            >
                {/* Mesh background */}
                <div className="absolute inset-0 bg-mesh-liquid opacity-[0.07] pointer-events-none" />
                ...
                {/* AI vs FAV Showdown */}
                {aiPick && fav && (
                    <div className="relative z-10 border-t border-surface-border/40 px-4 py-3 bg-gradient-to-r from-neon-green/[0.06] via-transparent to-value-orange/[0.04] caustic-spot">
                        <h4 className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-muted-light mb-2.5">
                            <Scale className="h-3 w-3 text-neon-green" /> The Matchup
                        </h4>
                        <div className="flex items-center justify-between gap-2">
                            <div className="flex-1 min-w-0 rounded-lg bg-neon-green/[0.06] border border-neon-green/10 px-2.5 py-2 shadow-[inset_0_1px_0_rgba(0,255,136,0.08)]">
                                <p className="text-[8px] text-neon-green uppercase mb-1 font-black tracking-widest">AI Pick</p>
                                <p className="text-sm font-black text-text-primary truncate leading-tight">{aiPick.name}</p>
                                <p className="font-mono-data text-[11px] font-bold text-neon-green/80 mt-0.5">{aiPick.odds}</p>
                            </div>
                            <div className="text-[9px] italic font-black text-muted-light/60 shrink-0">VS</div>
                            <div className="flex-1 min-w-0 rounded-lg bg-value-orange/[0.06] border border-value-orange/10 px-2.5 py-2 shadow-[inset_0_1px_0_rgba(255,149,0,0.08)] text-right">
                                <p className="text-[8px] text-value-orange uppercase mb-1 font-black tracking-widest">Market Fav</p>
                                <p className="text-sm font-black text-text-primary truncate leading-tight">{fav.name}</p>
                                <p className="font-mono-data text-[11px] font-bold text-value-orange/80 mt-0.5">{fav.odds}</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {/* Race Intel & Legacy */}
                <div className="relative z-10 border-t border-surface-border/40 px-4 py-3 bg-gradient-to-b from-terminal-bg/60 to-terminal-bg/80">
                ...
                {/* Go Arrow on hover */}
                <div className="absolute right-3 top-3 opacity-0 transition-all duration-200 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 glass-water rounded-full p-1.5 shadow-lg border-neon-green/20">
                    <ChevronRight className="h-3.5 w-3.5 text-neon-green" />
                </div>
            </div>
        </Link>
    );
